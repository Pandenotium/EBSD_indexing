...
def run_model(image_input):
    ...
    with tf.Session() as sess:
        ...
        #t = tf.get_default_graph().get_tensor_by_name('conv9/Relu:0')
        t = tf.get_default_graph().get_tensor_by_name('MaxPool2D_1/MaxPool:0')
        x = tf.get_default_graph().get_tensor_by_name('fc160/BiasAdd:0')
        g = sess.run(tf.gradients(x,t),feed_dict={eval_data: image_input}) #shape of g: (1,1,30,30,96)
        
        #print(np.array(g).shape)
        #plt.imshow(np.array(g)[0,0,:,:,0], cmap = 'Greys')
        
        
        im = np.array(g)
        
        for i in range(96):
            plt.imshow(im[0,0,:,:,i], cmap = 'Greys')
            fname = "gradientmaps/gmap_" + str(i) + ".png"
            plt.savefig(fname)

               
image_input,true = train_read(2,1)
run_model(image_input)

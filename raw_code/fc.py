...
def run_model(image_input):
    ...
    with tf.Session() as sess:
        ...
        for tensor_name in tensor_name_list:
            if ("fc" or "conv") not in tensor_name:
                print(tensor_name,'\n')
        '''
                
        mp_o = sess.run(tf.get_default_graph().get_tensor_by_name('MaxPool2D_1/MaxPool:0'),feed_dict={eval_data: image_input})
        #fc_o = sess.run(tf.get_default_graph().get_tensor_by_name('fc150/Relu:0'),feed_dict={eval_data: image_input})
        im = mp_o[:,:,:,1]
        im = np.reshape(im,[15,15])
        plt.imshow(im)

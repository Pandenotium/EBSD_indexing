...
def feature_map(image_input, layer, filter, save = False, grey = True, format = 'png'):
    ...
    :param layer: integer, indicates the convolutional layer. range:

    :param filter: integer, indicates the filter. range:
    ...
    with tf.Session() as sess:
        ...
        f_maps = sess.run(tf.get_default_graph().get_tensor_by_name('conv' + str(layer) + '/Relu:0'),feed_dict={eval_data: image_input})

        f_map = f_maps[:,:,:,filter]

        if layer in range(): # to find the size of filters
            f_size = 60
        if layer in range():
            f_size = 30
        if layer in range():
